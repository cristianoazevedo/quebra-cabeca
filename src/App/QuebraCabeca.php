...
class QuebraCabeca
{
    ...
    public static function realizarCortes()
    {
        ...
        foreach (self::$dimensoes as $posicao => $quadro) {
            imagecopy($imagem_base, $imagem_origem, 0, 0, $quadro[0], $quadro[2], self::TAMANHO_PECA, self::TAMANHO_PECA);
    ...
    private static function salvarSessao()
    {
        ...
        if (!$container->offsetGet('configuracoes')) {
            ...
            foreach (self::$configuracoes['padrao'] as $indice => $imagem) {
                if (self::$configuracoes['imagens'][$indice] == $imagem) {
                    self::$configuracoes['pecas_acerto'][$indice] = $imagem;
                }
            }
    ...
    public static function reorganizar($dados = [])
    {
        try {
            ...
            $nomeImagem = sprintf('%s.jpg', ($dados['para'] + 1));

            if ($de != $nomeImagem) {
                self::$configuracoes['errou']++;
            } else {
                self::$configuracoes['pecas_acerto'][$dados['para']] = $de;
            }
