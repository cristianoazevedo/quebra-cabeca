...
<div class="container">
    ...
    <?php
        $count = 0;
        foreach ($config['padrao'] as $index => $image) {
            if ($count == 0) {
                print '<tr>';
            }
    ?>
                <td style="padding: 0px 3px;">
                    <?php
                        if(isset($config['pecas_acerto'][$index]) && $config['pecas_acerto'][$index] == $image) {
                    ?>
                        <img src="../imagens/<?= $image ?>" alt="" class="responsive-img">
                    <?php
                        } else {
                    ?>
                        <img src="../interrogacao.jpg" alt="" class="responsive-img">
                    <?php
                        }
                    ?>
                </td>
    ...
    <?php
        ...
        foreach ($config['imagens'] as $index => $image) {
            ...
            if (!$config['fim_jogo'] && $config['padrao'][$index] != $image) {
                $params['de'] = $index;
    ...
    ?>
        ...
        <div class="col s6">
            <ul class="collection with-header">
                ...
                <li class="collection-item">Quantidade de erros <span class="badge"><?=$config['errou']?></span></li>
                <li class="collection-item">
                    <a href="controller/IndexController.php?method=limparSessao">Novo Jogo</a>
                </li>
            </ul>
            <div>
                <img src="../quebracabeca.jpg" class="responsive-img">
            </div>
        </div>
